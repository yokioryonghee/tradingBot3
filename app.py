...
commission_rate = st.sidebar.slider(
    "Transaction Cost (%)", 0.0, 0.5,
    TRANSACTION_COST_PERCENT * 100, format="%.2f"
) / 100
max_alloc_per_trade = st.sidebar.slider(
    "Max Allocation per Trade (%)", 1, 100,
    int(MAX_ALLOCATION_PCT_OF_INITIAL_CAPITAL_PER_TRADE * 100)
) / 100
take_profit = st.sidebar.slider(
    "Take Profit (%)", 0.0, 50.0,
    TAKE_PROFIT_PERCENT * 100, format="%.1f"
) / 100
stop_loss = st.sidebar.slider(
    "Stop Loss (%)", 0.0, 50.0,
    STOP_LOSS_PERCENT * 100, format="%.1f"
) / 100
...
with col_ml_train_date1:
    ml_train_start_date = st.date_input(
        "ML Model Training Start Date",
        value=datetime.datetime.strptime(START_DATE, '%Y-%m-%d').date(),
        max_value=end_date - datetime.timedelta(days=PREDICTION_HORIZON_DAYS * 2)
    )

with col_ml_train_date2:
    default_train_end_date = end_date - datetime.timedelta(days=PREDICTION_HORIZON_DAYS)
    min_allowed_date = ml_train_start_date + datetime.timedelta(days=PREDICTION_HORIZON_DAYS * 2)
    if default_train_end_date < min_allowed_date:
        default_train_end_date = min_allowed_date

    ml_train_end_date = st.date_input(
        "ML Model Training End Date",
        value=default_train_end_date,
        min_value=min_allowed_date
    )

# (이하 ML 학습, 백테스트, 성과 분석 로직 동일)
