# data_handler.py (최종 수정 버전 - app.py 파라미터 연동, fillna 강화, os import 확인, 1차원 데이터 오류 해결)
...
def calculate_indicators_and_target(df_raw, ema_short_period, ema_long_period, rsi_period,
    ...
    # 기술적 지표 계산
    # 핵심 수정: df['CLOSE']를 .squeeze()를 사용하여 확실하게 Series로 변환
    close_prices = df['CLOSE'].squeeze()

    df['EMA_short'] = ta.ema(close_prices, length=ema_short_period)
    print(f"DEBUG: EMA_short 계산 후 NaN 개수: {df['EMA_short'].isnull().sum()}. 현재 데이터 크기: {len(df)} 행")

    df['EMA_long'] = ta.ema(close_prices, length=ema_long_period)
    print(f"DEBUG: EMA_long 계산 후 NaN 개수: {df['EMA_long'].isnull().sum()}. 현재 데이터 크기: {len(df)} 행")

    df['RSI'] = ta.rsi(close_prices, length=rsi_period)
    print(f"DEBUG: RSI 계산 후 NaN 개수: {df['RSI'].isnull().sum()}. 현재 데이터 크기: {len(df)} 행")

    # --- MACD 계산 로직 강화: 반환되는 컬럼명 패턴을 확인하여 유연하게 할당 ---
    # MACD 계산도 close_prices (Series)를 사용합니다.
    macd_results = ta.macd(close_prices, fast=macd_fast_period, slow=macd_slow_period, signal=macd_signal_period)
    ...
    # ATR 계산도 Series를 사용합니다.
    atr_col_name = f'ATR_{14}'
    df[atr_col_name] = ta.atr(df['HIGH'].squeeze(), df['LOW'].squeeze(), close_prices, length=14)
    ...
    # --- 타겟 변수 'target' 계산 로직 ---
    # df['HIGH']도 .squeeze()를 사용합니다.
    df['future_high'] = df['HIGH'].squeeze().shift(-1).rolling(window=prediction_horizon_days, min_periods=1).max()
    print(f"DEBUG: future_high 계산 후 NaN 개수: {df['future_high'].isnull().sum()}. 현재 데이터 크기: {len(df)} 행")

    df['target'] = ((df['future_high'] / close_prices) - 1 >= target_rise_percent).astype(int)
...
# app.py에서 get_prepared_data로 임포트되므로 이 이름으로 유지합니다.
